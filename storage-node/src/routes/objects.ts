...
import { sendErrorResponse, createSuccessResponse } from "../services/response/apiResponse";
import { HttpError } from "../utils/HttpError";
import { downloadObject, uploadObject } from "../services/objects/objectService";
...
const objects: FastifyPluginAsync = async (fastify, opts): Promise<void> => {
  ...
  fastify.get<{
    ...
    try {
      ...
      return sendErrorResponse(reply, 500, "파일 다운로드 중 오류가 발생했습니다", {
  ...
  fastify.put<{
    ...
    try {
      ...
      return sendErrorResponse(reply, 500, "파일 업로드 중 오류가 발생했습니다", {
        error: error instanceof Error ? error.message : "알 수 없는 오류",
      });
    }
  });
